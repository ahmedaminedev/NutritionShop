...
const KPICard: React.FC<{ title: string; value: string; trend?: number; icon: React.ReactNode; color: string }> = ({ title, value, trend, icon, color }) => (
    <div className={`bg-brand-gray border border-gray-800 p-6 relative overflow-hidden group hover:border-${color}-500/50 transition-all duration-500`}>
        <div className={`absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 opacity-10 group-hover:opacity-20 transition-opacity text-${color}-500`}>{icon}</div>
        <div className={`w-1 h-8 bg-${color}-500 absolute left-0 top-1/2 -translate-y-1/2`}></div>
        ...
        <div className="flex items-end gap-3">
            ...
            {trend !== undefined && (
                <div className={`flex items-center text-[10px] font-bold ${trend >= 0 ? 'text-brand-neon' : 'text-brand-alert'} mb-1`}>
...
export const DashboardHomePage: React.FC<DashboardProps> = ({ orders, products, messages }) => {
    ...
    // Fake trend data for visual impact
    const stats = [
        { title: 'Chiffre d\'Affaires', value: `${revenue.toFixed(0)} DT`, trend: 12, icon: <CreditCardIcon />, color: 'brand-neon' },
        { title: 'Commandes Total', value: orders.length.toString(), trend: 5, icon: <ShoppingBagIcon />, color: 'blue' },
        { title: 'Stock Critique', value: products.filter(p => p.quantity < 5).length.toString(), trend: -8, icon: <ChartPieIcon />, color: 'red' },
        { title: 'Transmissions', value: messages.filter(m => !m.read).length.toString(), trend: 0, icon: <InboxIcon />, color: 'amber' }
    ];

    return (
        <div className="space-y-8 animate-fadeIn p-8 bg-[#050505] min-h-screen">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-gray-800 pb-8 gap-4">
                <div>
                    ...
                    <p className="text-gray-500 font-mono text-xs uppercase tracking-[0.3em] mt-2">Protocoles de surveillance en temps réel</p>
                </div>
                <div className="flex items-center gap-3 bg-brand-gray px-4 py-2 border border-gray-800">
                    <span className="w-2 h-2 bg-brand-neon rounded-full animate-pulse shadow-[0_0_10px_#ccff00]"></span>
                    <span className="text-[10px] font-black text-white uppercase tracking-widest">Système Opérationnel</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Revenue Chart */}
                <div className="lg:col-span-2 bg-brand-gray border border-gray-800 p-8 relative overflow-hidden">
                ...
                {/* Stock Warning Box */}
                <div className="bg-brand-gray border border-gray-800 p-8">
                    ...
                    <div className="space-y-4 max-h-[350px] overflow-y-auto custom-scrollbar pr-2">
                        {products.filter(p => p.quantity < 10).map(p => (
                            <div key={p.id} className="flex justify-between items-center p-3 bg-black border border-gray-800 group hover:border-brand-neon transition-colors">
                                <div className="min-w-0 flex-1">
                                    <p className="text-[10px] font-bold text-gray-400 truncate uppercase">{p.name}</p>
                                    <p className="text-[9px] text-gray-600 font-mono">{p.brand}</p>
                                </div>
                                <div className="text-right ml-4">
                                    <p className={`text-xs font-black ${p.quantity === 0 ? 'text-brand-alert' : 'text-white'}`}>{p.quantity} UN</p>
                                    <p className="text-[8px] text-gray-500 uppercase font-bold">Stock</p>
                                </div>
                            </div>
                        ))}
                    </div>
