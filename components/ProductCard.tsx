...
import { ShoppingBagIcon, EyeIcon, HeartIcon, PlusIcon } from './IconComponents';
        ...
        >
            ...
            <div className="relative aspect-[4/5] overflow-hidden bg-gray-50 dark:bg-black/40">
                ...
                <div className="absolute top-4 left-0 flex flex-col gap-1.5 z-10">
                    ...
                    {product.promo && (
                        <div className="bg-black dark:bg-white text-white dark:text-black text-[10px] font-black px-3 py-1 slant shadow-lg">
                ...
                {/* Quick Actions */}
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 z-20 backdrop-blur-[2px]">
                    <button 
                        onClick={(e) => { e.stopPropagation(); onPreview(product); }}
                        className="w-12 h-12 bg-white text-black flex items-center justify-center hover:bg-brand-neon transition-all transform -translate-y-4 group-hover:translate-y-0 duration-300"
                        title="Aperçu Rapide"
                    >
                    ...
                    <button 
                        ...
                        className={`w-12 h-12 bg-black text-white border border-white/20 flex items-center justify-center hover:text-brand-neon transition-all transform translate-y-4 group-hover:translate-y-0 duration-300 ${isFav ? 'text-brand-neon border-brand-neon' : ''}`}
            ...
            <div className="p-6 flex flex-col flex-grow relative z-10">
                <div className="flex justify-between items-center mb-3">
                    ...
                    <div className={`h-1.5 w-1.5 rounded-full ${isOutOfStock ? 'bg-brand-alert' : 'bg-brand-neon shadow-[0_0_8px_#ccff00]'}`}></div>
                </div>

                <h3 className="font-serif font-black text-xl leading-tight mb-4 text-gray-900 dark:text-white group-hover:text-brand-neon transition-colors line-clamp-2 uppercase italic tracking-tighter">
                    {product.name}
                </h3>
                ...
                <div className="mt-auto pt-4 border-t border-gray-100 dark:border-gray-800">
                    <div className="flex items-baseline gap-2 mb-5">
                        <span className="text-2xl font-black text-gray-900 dark:text-white font-mono tracking-tighter">
                            {product.price.toFixed(3)} <span className="text-[10px] font-bold text-brand-neon">TND</span>
                        </span>
                        {product.oldPrice && (
                            <span className="text-xs text-gray-400 line-through font-mono">{product.oldPrice.toFixed(3)}</span>
                        )}
                    </div>
                    ...
                    <button 
                        ...
                        className="w-full bg-black dark:bg-white text-white dark:text-black font-black uppercase text-[10px] tracking-[0.2em] py-4 flex items-center justify-center gap-3 hover:bg-brand-neon hover:text-black dark:hover:bg-brand-neon dark:hover:text-black transition-all slant disabled:opacity-30 disabled:cursor-not-allowed shadow-xl group/btn"
                    >
                        <span className="slant-reverse block flex items-center gap-2">
                            {isOutOfStock ? 'INDISPONIBLE' : 'DÉPLOYER AU SET'}
                            {!isOutOfStock && <PlusIcon className="w-3.5 h-3.5 transition-transform group-hover/btn:rotate-90" />}
                        </span>
                    </button>
                </div>
            </div>
